...
public class UpdateProductTests
{
    ...
    public async Task Handle_ShouldReturnSuccessResult_WhenProductIsUpdated()
    {
        ...
        var existingProduct =
            Product.Create(ProductId.New(), "Test Product", null, 10, ProductCategory.Acompanhamento, null);
        _productRepository.FindByIdAsync(Arg.Any<ProductId>(), Arg.Any<CancellationToken>()).Returns(existingProduct);
        ...
        var updatedProduct = Product.Create(new ProductId(command.Id), command.Name, command.Description, command.Price,
